            ...
            <div className="mt-16 flex justify-center">
              <fieldset aria-label="Payment frequency">
                <div className="grid grid-cols-2 gap-x-1 rounded-full bg-white/5 p-1 text-center text-xs/5 font-semibold text-white">
                  <label className="group relative rounded-full px-2.5 py-1 has-checked:bg-emerald-500">
                    <input
                ...
                <div className="p-8 lg:pt-12 xl:p-10 xl:pt-14">
                  ...
                  <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:flex-col lg:items-stretch">
                    ...
                    <button
                      ...
                      className="w-full rounded-md bg-white/10 px-3 py-2 text-center text-sm/6 font-semibold text-white group-data-featured/tier:bg-emerald-600 group-data-featured/tier:shadow-xs hover:bg-white/20 group-data-featured/tier:hover:bg-emerald-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white group-data-featured/tier:focus-visible:outline-emerald-600"
                    >
                      Buy this plan
                    </button>
                  </div>
                    ...
                    >
                      {tier.highlights.map((mainFeature) => (
                        <li key={mainFeature} className="flex gap-x-3 py-2">
                          <CheckIcon
                            aria-hidden="true"
                            className="h-6 w-5 flex-none text-gray-500 group-data-featured/tier:text-emerald-600"
                          />
                          {mainFeature}
                        </li>
                      ))}
            ...
            <div className="mx-auto max-w-2xl space-y-16">
              {tiers.map((tier) => (
                <div key={tier.id} className="border-t border-gray-900/10">
                  <div
                    className={classNames(
                      tier.featured ? 'border-emerald-600' : 'border-transparent',
                      '-mt-px w-72 border-t-2 pt-10 md:w-80',
                    )}
                  >
                    <h3
                      className={classNames(
                        tier.featured ? 'text-emerald-600' : 'text-gray-900',
                    ...
                    {sections.map((section) => (
                      <div key={section.name}>
                        ...
                        <div className="relative mt-6">
                          ...
                          <div
                            className={classNames(
                              tier.featured ? 'ring-2 ring-emerald-600' : 'ring-1 ring-gray-900/10',
                              ...
                              {section.features.map((feature) => (
                                ...
                                >
                                  ...
                                  <dd className="flex items-center justify-end sm:justify-center sm:px-4">
                                    {typeof feature.tiers[tier.name as keyof typeof feature.tiers] as string === 'string' ? (
                                      <span
                                        className={tier.featured ? 'font-semibold text-emerald-600' : 'text-gray-900'}
                                      >
                                      ...
                                      <>
                                        {feature.tiers[tier.name as keyof typeof feature.tiers] === true ? (
                                          <CheckIcon aria-hidden="true" className="mx-auto size-5 text-emerald-600" />
                                        ) : (
                              ...
                              <td key={tier.id} className="relative w-1/4 px-4 py-0 text-center">
                                <span className="relative size-full py-3">
                                  {typeof feature.tiers[tier.name as keyof typeof feature.tiers] as string === 'string' ? (
                                    <span
                                      className={classNames(
                                        tier.featured ? 'font-semibold text-emerald-600' : 'text-gray-900',
                                        'text-sm/6',
                                      )}
                                    >
                                      {feature.tiers[tier.name as keyof typeof feature.tiers]}
